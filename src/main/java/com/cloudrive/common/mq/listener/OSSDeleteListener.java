...
public class OSSDeleteListener {
    ...
    public void listenOssDelete(String path){
        //在删除对应OSS文件之前，先检查是否有其他引用（相同哈希值的文件），状态为未彻底删除（0,1,2）
        long referenceCount = fileInfoMapper.countByPathAndIsDeletedFalse(path);
        // 如果没有当前文件引用，则从存储中删除
        if (referenceCount < 1) {
            StorageService storageService = storageServiceFactory.getStorageService();
            storageService.deleteFile(path);
            logger.info("从存储中删除了文件: {}", path);
        } else {
            logger.info("不执行物理删除，因为文件路径: {}, 当前引用计数: {}", path, referenceCount);
        }
    }
}
